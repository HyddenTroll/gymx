        ...
        {profil && (
          <div className="card p-3 flex gap-2 flex-wrap">
            <span className="text-[10px] font-semibold px-2 py-1 rounded" style={{ backgroundColor: "var(--color-gymx-border)", color: "var(--color-gymx-text)" }}>
              {profil.niveau === "debutant" ? "Débutant" : profil.niveau === "intermediaire" ? "Intermédiaire" : "Avancé"}
            </span>
            <span className="text-[10px] font-semibold px-2 py-1 rounded" style={{ backgroundColor: "var(--color-gymx-border)", color: "var(--color-gymx-text)" }}>
            ...
            <span className="text-[10px] font-semibold px-2 py-1 rounded" style={{ backgroundColor: "var(--color-gymx-border)", color: "var(--color-gymx-text)" }}>
              {profil.materiel === "salle" ? "Salle" : profil.materiel === "halteres" ? "Haltères" : "Corps"}
            </span>
          </div>
        )}
        ...
        {detail ? (
            ...
            <div className="card p-5 space-y-4">
              ...
              {detail.match_pct > 0 && (
                <div className="flex items-center gap-2">
                  <span className="text-xs font-semibold" style={{ color: "var(--color-gymx-accent)" }}>
                    {detail.match_pct}% de match
                  </span>
              ...
              <button onClick={() => handleChanger(detail)} disabled={creating}
                className="w-full py-3.5 rounded-xl font-semibold text-sm touch-target disabled:opacity-30"
                style={{ backgroundColor: "var(--color-gymx-accent)", color: "#0a0a0b" }}>
                {creating ? "Création…" : "Choisir ce programme"}
              </button>
            </div>
          </div>
        ) : (
          <div className="space-y-2">
            {recommandations.map((rec, i) => (
              <button key={rec.programme.id} onClick={() => setDetail(rec)}
                className={`w-full card p-4 text-left space-y-2 transition-all active:scale-[0.98] touch-target ${
                  !rec.eligible ? "opacity-40" : ""
                }`}>
                ...
                <div className="flex gap-1.5 flex-wrap">
                  <span className="text-[10px] font-semibold px-2 py-0.5 rounded" style={{ backgroundColor: "var(--color-gymx-border)", color: "var(--color-gymx-text)" }}>
                    {rec.programme.jours_par_semaine}j/sem
                  </span>
                  <span className="text-[10px] font-semibold px-2 py-0.5 rounded" style={{ backgroundColor: "var(--color-gymx-border)", color: "var(--color-gymx-text)" }}>
                    {rec.programme.duree_semaines} sem
                  </span>
                  {rec.programme.objectifs.map((o) => (
                    <span key={o} className="text-[10px] font-semibold px-2 py-0.5 rounded" style={{ backgroundColor: "var(--color-gymx-accent-dim)", color: "var(--color-gymx-accent)" }}>
                      {o === "force" ? "Force" : o === "muscle" ? "Muscle" : "Recomp"}
                    </span>
                  ))}
                  {rec.programme.tags.slice(0, 2).map((t) => (
                    <span key={t} className="text-[10px] px-2 py-0.5 rounded" style={{ backgroundColor: "var(--color-gymx-border)", color: "var(--color-gymx-muted)" }}>
                      {t}
                    </span>
                  ))}
                </div>
